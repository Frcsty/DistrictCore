...
public class PouchStorage {
    ...
    public final void loadPouches(final DistrictCore core) {
        final ConfigurationSection pouchesSection = core.getSectionLoader().getSection("pouches");
        if (pouchesSection == null) {
            return;
        }

        for (String pouchString : pouchesSection.getKeys(false)) {
            final ConfigurationSection pouchItemSection = pouchesSection.getConfigurationSection(pouchString + ".item");
            final ConfigurationSection pouchSection = pouchesSection.getConfigurationSection(pouchString);
